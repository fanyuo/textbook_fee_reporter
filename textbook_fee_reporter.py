...
def process_book_excel(file_path, target_class):
    ...
    try:
        ...
        required_columns = {
            ...
            '课程名称': df.iloc[header_row].astype(str).str.contains("课程名称"),
            '书号': df.iloc[header_row].astype(str).str.contains("书号"),
            '教材名称': df.iloc[header_row].astype(str).str.contains("教材名称"),
            '出版社': df.iloc[header_row].astype(str).str.contains("出版社"),
            '折扣价': df.iloc[header_row].astype(str).str.contains("折扣价")
        }

        # 验证必要列是否存在
        missing_cols = [col for col, mask in required_columns.items() if not mask.any()]
        if missing_cols:
        ...
        while current_row < len(df):
            ...
            book_info = {
                '序号': serial_number,
                '课程名称': df.iloc[current_row, col_indices['课程名称']],
                '书号': df.iloc[current_row, col_indices['书号']],
                '教材名称': df.iloc[current_row, col_indices['教材名称']],
                '出版社': df.iloc[current_row, col_indices['出版社']],
                '折扣价': float(df.iloc[current_row, col_indices['折扣价']]) if pd.notna(df.iloc[current_row, col_indices['折扣价']]) else 0.0
            }
...
def process_student_excel(file_path, target_class):
    ...
    try:
        ...
        header_row = df[0].astype(str).str.contains("姓名")
        # 检测所有需要的列
        required_columns = {
            ...
            '课程名称': df.iloc[header_row].astype(str).str.contains("课程名称"),
            '教材名称': df.iloc[header_row].astype(str).str.contains("教材名称"),
            '出版社名称': df.iloc[header_row].astype(str).str.contains("出版社名称"),
            'ISBN号': df.iloc[header_row].astype(str).str.contains("ISBN号")
        }
        ...
        for row in range(header_row + 1, len(df)):
            ...
            if target_class in student_class:
                record = {
                    ...
                    '课程名称': str(df.iloc[row, col_indices['课程名称']]).strip(),
                    '教材名称': str(df.iloc[row, col_indices['教材名称']]).strip(),
                    '出版社名称': str(df.iloc[row, col_indices['出版社名称']]).strip(),
                    'ISBN号': str(df.iloc[row, col_indices['ISBN号']]).strip()
                }
                # 确保所有字段都有值
...
def calculate_student_payments(book_data, student_records):
    ...
    计算每个学生的购书总费用
    参数:
        book_data: 教材清单 [{'书号':xx, '教材名称':xx, '折扣价':xx}, ...]
        student_records: 学生购书记录 [{'姓名':xx, '教材名称':xx, 'ISBN号':xx}, ...]
    返回:
        [{'姓名':xx, '购书费用':xx}, ...]
    """
    try:
        ...
        for student in student_records:
            isbn = str(student['ISBN号']).strip()
            book_name = str(student['教材名称']).strip()
            fmt1_name = str_format_1(book_name)
            fmt2_name = str_format_2(book_name)

            matched_price = None
            if isbn in price_maps['isbn']:
                matched_price = price_maps['isbn'][isbn]
            elif book_name in price_maps['raw_name']:
            ...
            if matched_price is not None:
                ...
                unmatched_records.append(f"学生[{student['姓名']}] 教材[{book_name}] ISBN[{isbn}]")

        if unmatched_records:
            error_msg = "以下教材无法匹配价格：\n"
            error_msg += "\n".join(unmatched_records)
            error_msg += "\n匹配顺序：1.ISBN 2.原始名称 3.去除括号 4.去除括号及内容"
...
def generate_report(book_data, student_records, student_payments):
    ...
    print("=" * 80)
    for book in book_data:
        print(f"序号: {book['序号']}, 课程: {book['课程名称']}, 教材: {book['教材名称']}, 出版社: {book['出版社']}, 价格: ￥{book['折扣价']:.2f}")
